...
from btm_db import db_healthcheck, generate_code, add_access_codes, list_access_codes, export_access_codes
...
count = int(st.number_input("How many codes?", min_value=1, max_value=200, value=1))
...
for r in rows:
    ...
    st.write(f"{r['code']} — {status} — note: {r.get('note') or '—'}")

st.markdown("---")
st.subheader("Export CSV")

export_inactive = st.checkbox("Include inactive in export", value=True)
if st.button("Prepare CSV"):
    data = export_access_codes(include_revoked=export_inactive)
    df = pd.DataFrame(data)
    st.download_button(
        "Download access_codes.csv",
        df.to_csv(index=False).encode("utf-8"),
        file_name="access_codes.csv",
        mime="text/csv",
    )
